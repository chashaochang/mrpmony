...
int startVmrpWithPath(const char *filename, const char *extName) {
    uc = initVmrp();
    if (uc == NULL) {
    ...
    if (bridge_ext_init(uc) != UC_ERR_OK) {
        printf("bridge_ext_init fail.\n");
        stopVmrp();
        return MR_FAILED;
    }

    if (bridge_dsm_init(uc) != MR_SUCCESS) {
        printf("bridge_dsm_init fail.\n");
        stopVmrp();
        return MR_FAILED;
    }
    ...
    int32_t ret = bridge_dsm_mr_start_dsm(uc, (char *)filename, (char *)extName, NULL);
...
int startVmrp() {
    return startVmrpWithPath("dsm_gm.mrp", "start.mr");
}
